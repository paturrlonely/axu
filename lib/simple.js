...
`.trim()
                ...
                return await conn.sendMessage(jid, { contacts: { ...options, displayName: (contacts.length > 1 ? `${contacts.length} kontak` : contacts[0].displayName) || null, contacts }}, { quoted, ...options, ...ephemeral })
            }
        },
        sendGroupV4Invite: {
            /**
        ...
        sendButton: {
          async value(jid, title, body, footer, buttons, m, quoted) {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          /* agar support all tag */
          contextInfo: {
                  mentionedJid: conn.parseMention(body),
                }
        })
    }
  }
...
return await conn.relayMessage(jid, msgs.message, {
  messageId: m.key.id
        ...
        sendButtonImage: {
           async value(jid, body, footer, image_url, buttons, m, quoted) {
             let msgs = generateWAMessageFromContent(jid, {
           viewOnceMessage: {
        message: {
        "messageContextInfo": {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          header: proto.Message.InteractiveMessage.Header.create({
                ...(await prepareWAMessageMedia({ image : { url: image_url}}, { upload: conn.waUploadToServer })), 
                  title: ``,
                  gifPlayback: true,
                  subtitle: '',
                  hasMediaAttachment: false  
...
`.trim()
                ...
                return await conn.sendMessage(jid, { ...options, contacts: { ...options, displayName: (contacts.length >= 2 ? `${contacts.length} kontak` : contacts[0].displayName) || null, contacts }}, { quoted, ...options, ...ephemeral })
            },
            enumerable: true
        },
        reply: {
            ...
            value(jid, text = '', quoted, options) {
                return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text }, { quoted, ...options, ...ephemeral })
            }
        },
        adReply: {
        ...
        cMod: {
            ...
            value(jid, message, text = '', sender = conn.user.jid, options = {}) {
                ...
                if (typeof content !== 'string') {
                    msg[mtype] = { ...content, ...options, ...ephemeral }
