...
let handler = async (m, {
	...
	try {
		...
		console.log(e)
		try {
			let anuk = await (await fetch(`https://api.botcahx.eu.org/api/dowloader/yt?url=${args[0]}&apikey=${btc}`)).json()
			let txt = `*${anuk.result.title}*\n\n` +
				`⌲ Resolution : 360p\n` +
				`⌲ Duration : ${anuk.result.duration}`
			await conn.sendFile(m.chat, anu.result.mp4, `${anuk.result.title}.mp4`, txt, m)
		} catch (e) {
			try {
