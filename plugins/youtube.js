...
handler.before = async (m, {
	...
	if (inputNumber >= 1 && inputNumber <= limitedVideos.length) {
		...
		try {
			let {
				result: yt
			} = await (await fetch(`https://api.betabotz.eu.org/api/download/ytmp3?url=${selectedUrl}&apikey=${lann}`)).json()
...
`;
			...
			try {
				let {
					result: btc
				} = await (await fetch(`https://api.botcahx.eu.org/api/dowloader/yt?url=${selectedUrl}&apikey=${global.btc}`)).json()
				let infoText = `*AUDIO INFORMATION*
- *Title:* ${btc.title}
